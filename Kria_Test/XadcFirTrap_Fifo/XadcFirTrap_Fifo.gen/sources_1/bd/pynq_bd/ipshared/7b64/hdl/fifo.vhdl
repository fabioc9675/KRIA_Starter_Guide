...
entity FIFO is
   ...
   port (
      ...
      wcount_o     : out std_logic_vector(15 downto 0); -- Elements in the write side
      -- read side
      ...
      rcount_o     : out std_logic_vector(15 downto 0)  -- Elements in the read side
   );
end entity FIFO;
...
architecture RTL of FIFO is
   ...
   signal wcount,  rcount  : unsigned(AWIDTH downto 0);
...
begin
   ...
   g_async: if ASYNC generate
      ...
      i_sync_rd2wr: entity work.graysync
      ...
      i_sync_wr2rd: entity work.graysync
      generic map(WIDTH => AWIDTH+1, DEPTH => 2)
   ...
   wcount      <= diff_ptr(wr_ptr, rd_in_wr_ptr);
   full        <= '1' when wcount = EVEN_DEPTH else '0';
   full_o      <= full;
   afull_o     <= '1' when wcount >= EVEN_DEPTH-AFULLOFFSET else '0';
   ...
   rcount      <= diff_ptr(wr_in_rd_ptr, rd_ptr);
   empty       <= '1' when rcount = 0 else '0';
   empty_o     <= empty;
   aempty_o    <= '1' when rcount <= AEMPTYOFFSET else '0';
   ...
   process(wcount, rcount)
   begin
      wcount_o <= (others => '0');
      rcount_o <= (others => '0');
      if AWIDTH < 16 then
         wcount_o(AWIDTH downto 0) <= std_logic_vector(wcount);
         rcount_o(AWIDTH downto 0) <= std_logic_vector(rcount);
      else
         if wcount < x"FFFF" then
            wcount_o <= std_logic_vector(wcount(15 downto 0));
         else
            wcount_o <= (others => '1');
         end if;
         if rcount < x"FFFF" then
            rcount_o <= std_logic_vector(rcount(15 downto 0));
         else
            rcount_o <= (others => '1');
         end if;
      end if;
